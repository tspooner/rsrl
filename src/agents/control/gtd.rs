...
pub struct GreedyGQ<S, M: Projector<S>, P: QPolicy<S>> {
...
impl<S, M: Projector<S>, P: QPolicy<S>> GreedyGQ<S, M, P> {
    pub fn new<T1, T2, T3>(
...
impl<S, M: Projector<S>, P: QPolicy<S>> Handler<Transition<S, usize>> for GreedyGQ<S, M, P> {
...
impl<S, M: Projector<S>, P: QPolicy<S>> Controller<S, usize> for GreedyGQ<S, M, P> {
    fn pi(&mut self, s: &S) -> usize {
        Greedy.sample_qs(&s, self.fa_theta.evaluate(s).unwrap().as_slice().unwrap())
    }

    fn mu(&mut self, s: &S) -> usize {
        self.policy.sample_qs(&s, self.fa_theta.evaluate(s).unwrap().as_slice().unwrap())
    }
}
