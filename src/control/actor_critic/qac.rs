...
impl<S: Clone, Q, C, P> Handler<Transition<S, Action>> for QAC<S, Q, C, P>
where
    Q: QFunction<S>,
    C: TDPredictor<S>,
    P: FinitePolicy<S>,
...
impl<S: Clone, Q, C, P> Controller<S, Action> for QAC<S, Q, C, P>
where
    Q: QFunction<S>,
    C: TDPredictor<S>,
    P: FinitePolicy<S>,
{
    fn pi(&mut self, s: &S) -> Action { self.target.sample(&s) }

    fn mu(&mut self, s: &S) -> Action { self.policy.sample(s) }
}
