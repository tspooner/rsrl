use core::{Shared, make_shared};
use fa::{Approximator, VFunction, QFunction, EvaluationResult, UpdateResult};
use geometry::Vector;
...
impl MockQ {
    pub fn new(output: Option<Vector<f64>>) -> Self {
        MockQ {
            output: output,
        }
    }
    ...
    pub fn set_output(&mut self, output: Vector<f64>) {
...
impl Approximator<Vec<f64>> for MockQ {
    ...
    fn update(&mut self, p: &Vec<f64>, errors: Vector<f64>) -> UpdateResult<()> {
        Ok(())
    }
}
